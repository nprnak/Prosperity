...
const review = (appId) => {
    router.post(route('reviewer.applications.review', appId), {}, { preserveScroll: true });
};

const reject = (appId) => {
    const reason = (rejecting[appId] || '').trim();
    if (!reason) return;
    router.post(route('reviewer.applications.reject', appId), { rejection_reason: reason }, { preserveScroll: true });
};
</script>
...
<template>
  ...
  <AuthenticatedLayout>
    <template #header><h2 class="font-semibold text-xl">Reviewer Dashboard</h2></template>
    <div class="py-8 max-w-5xl mx-auto space-y-4 px-4">
      ...
      <p class="text-sm text-gray-600">Applications with verified payments awaiting review. Reviewed applications move on to the verifier.</p>
      ...
      <div v-for="app in applications" :key="app.id" class="bg-white p-4 rounded shadow">
        <div class="flex flex-wrap items-center justify-between gap-2">
          ...
          <button class="px-3 py-1 bg-blue-600 text-white rounded" @click="review(app.id)">Mark Reviewed</button>
        </div>
