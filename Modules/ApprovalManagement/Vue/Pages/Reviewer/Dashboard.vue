...
const props = defineProps({
    ...
    reviewedByMe: { type: Object, default: () => ({ data: [], links: [] }) },
...
const tabs = [
    ...
    { key: 'reviewed', label: 'Reviewed by you', get count() { return props.reviewedByMe.total; } },
];

const applySearch = () => {
    router.get(route('reviewer.dashboard'), { q: search.value || undefined }, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
...
<template>
    ...
    <PanelLayout>
        ...
        <div class="mx-auto max-w-5xl space-y-6">
            <div>
                <h3 class="text-lg font-semibold text-slate-900">Application review</h3>
            ...
            <section v-show="activeTab === 'pending'" class="space-y-4">
                ...
                >
                    Nothing waiting on you. Applications appear here once a verifier signs them off.
            ...
            <section v-show="activeTab === 'reviewed'" class="space-y-4">
                <p
                    v-if="!reviewedByMe.data.length"
                    class="max-w-[70ch] rounded-xl bg-white p-6 text-sm text-gray-700 shadow-sm ring-1 ring-gray-200"
                >
                    You have not reviewed any applications yet.
                </p>

                <article
                    v-for="app in reviewedByMe.data"
                    :key="`reviewed-${app.id}`"
                ...
                <Pagination :meta="reviewedByMe" label="reviewed applications" />
            </section>
        </div>
    </PanelLayout>
</template>
