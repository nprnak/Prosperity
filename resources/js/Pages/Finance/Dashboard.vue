...
const props = defineProps({ applications: Array, status: String });
...
const initForm = (appId) => {
  if (!paymentForms[appId]) {
    paymentForms[appId] = useForm({
      amount: '', payment_mode: 'cash', payment_date: '', bank_name: '', payment_reference_no: '', cheque_no: '', holding_id_no: '', id_type: 'citizenship', notes: ''
    });
  }
